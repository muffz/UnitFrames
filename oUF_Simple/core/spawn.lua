...
if L.F.CreateHealStyle then oUF:RegisterStyle(A.."Heal", L.F.CreateHealStyle) end
...
--spawn heal
if L.F.CreateHealStyle then
  oUF:SetActiveStyle(A.."Heal")
  oUF:SpawnHeader(
    A.."HealHeader",
    L.C.heal.setup.template,
    L.C.heal.setup.visibility,
    "showPlayer", L.C.heal.setup.showPlayer,
    "showSolo",   L.C.heal.setup.showSolo,
    "showParty",  L.C.heal.setup.showParty,
    "showRaid",   L.C.heal.setup.showRaid,
    "point",      L.C.heal.setup.point,
    "xOffset",    L.C.heal.setup.xOffset,
    "yOffset",    L.C.heal.setup.yOffset,
    "oUF-initialConfigFunction", ([[
      self:SetWidth(%d)
      self:SetHeight(%d)
      self:GetParent():SetScale(%f)
    ]]):format(L.C.heal.size[1], L.C.heal.size[2], L.C.heal.scale)
  ):SetPoint(unpack(L.C.heal.point))
end
